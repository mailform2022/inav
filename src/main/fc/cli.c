...
static void cliStatus(char *cmdline)
{
    ...
    if (vtxSettingsConfig()->frequencyGroup == FREQUENCYGROUP_3G3) {
        ...
        cliPrintf("]");
        cliPrintLinefeed();
        cliPrintf("VTX 3G3: band=%d chan=%d pwr=%d gridFreq=%d powerMode=%d clrPit=%d keepalive=%d chanMode=%d txTotal=%lu",
        ...
        cliPrintLinefeed();
        cliPrintf("VTX 3G3 chan: setMode=%d interByteMs=%d settleMs=%d saPulldown=%d freqFix=%d lastTx=[",
                vtxConfig()->vtx3g3ChanSetMode ? 1 : 0,
                vtxConfig()->vtx3g3ChanInterByteMs, vtxConfig()->vtx3g3ChanSettleMs,
                vtxConfig()->vtx3g3SaPulldown ? 1 : 0,
                vtxConfig()->vtx3g3ChanFreqFix ? 1 : 0);
        for (int i = 0; i < sa3G3LastChanLen; i++) {
            cliPrintf("%s%02X", (i ? " " : ""), sa3G3LastChanBuf[i]);
        }
        cliPrintf("]");
        cliPrintLinefeed();
        cliPrintf("VTX 3G3 tramp pwr codes: %d %d %d",
                vtxConfig()->vtx3g3TrampPwrCode[0], vtxConfig()->vtx3g3TrampPwrCode[1],
                vtxConfig()->vtx3g3TrampPwrCode[2]);
        cliPrintLinefeed();
        static const char * const detectNames[] = {
            "none", "forced", "range", "power", "protocol", "fallback"
        };
