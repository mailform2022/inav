...
#include "io/vtx_control.h"   // или vtx_common.h
...
} else if (cmdMSP == MSP_VTX_TABLE_CUSTOM) {
    // Приём кастомной таблицы VTX
    uint8_t name[21];
    for (int i = 0; i < 20; i++) {
        name[i] = sbufReadU8(src);
    }
    name[20] = '\0';
    uint8_t bandCount = sbufReadU8(src);
    customVtxTable.bandCount = bandCount;
    for (int b = 0; b < bandCount; b++) {
        for (int i = 0; i < 8; i++) {
            customVtxTable.bands[b].name[i] = sbufReadU8(src);
        }
        customVtxTable.bands[b].name[8] = '\0';
        for (int ch = 0; ch < 8; ch++) {
            customVtxTable.bands[b].frequencies[ch] = sbufReadU16(src);
        }
    }
    uint8_t powerCount = sbufReadU8(src);
    customVtxTable.powerCount = powerCount;
    for (int p = 0; p < powerCount; p++) {
        customVtxTable.powerlevels[p] = sbufReadU16(src);
    }
    customVtxTableLoaded = true;
    ret = MSP_RESULT_ACK;
} else {
